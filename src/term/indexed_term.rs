...
// impl FromWithIndex<Term> for IndexedTerm {
//     fn from_with_index(item: Term, index: Arc<RwLock<Model>>) -> Self {
//         IndexedTerm::new(&item, index)
//     }
// }
...
//             return IndexedTerm { id, index: index_copy };
//         }

//         // Write a new term into model store and return its id to create an IndexedTerm.
//         let mut writable_model = index.write().unwrap();
...
//         // Cannot return term reference before reference won't live longer than read guard.
//         let readable = self.index.read().unwrap();
//         let term_ref = readable.get_term_by_id(self.id).unwrap();
//         return term_ref.clone();
//     }
...
//         let read = self.get_read_model();
//         let term_ref = read.get_term_by_id(self.id).unwrap();
//         match term_ref {
//             Term::Composite(c) => {
...
//         if let Term::Composite(ref mut c) = new_term {
//             for arg in c.arguments.iter_mut() {
//                 let arg_ref = arg.clone();
//                 let mut indexed_arg = IndexedTerm::new(arg_ref.borrow(), self.index.clone());
...
//             Term::Composite(sc) => {
//                 let read = self.get_read_model();
//                 let other_term = read.get_term_by_id(term.id).unwrap();
//                 match other_term {
//                     Term::Composite(tc) => {
...
//                             let x: Self::Output = IndexedTerm::new(x_ref, self.index.clone());
//                             let y: Self::Output = IndexedTerm::new(y_ref, self.index.clone());
                            
//                             let read = self.get_read_model();
//                             let x_ref = read.get_term_by_id(x.id).unwrap();
//                             match x_ref {
//                                 Term::Atom(xa) => {
...
//     {
//         let read = self.get_read_model();
//         let term = read.get_term_by_id(self.id).unwrap();
//         term.is_groundterm()
//     }
