...
class DifferentialDataflowExecuter:
    def __init__(self, model):
        self.model = model
        self.engine = DDExecuter(model)

    def compile(self):
        for cluster in self.model.domain.stratified_rules:
            for rule in cluster:
                self.add_rule(rule)

        changes = {}
        for fact in self.model.initial_facts:
            changes[fact] = 1
    ...
    def add_changes(self, changes):
        for fact in changes:
            count = changes[fact]
            index = self.model.type_index_map[fact.sort.name]
            index.add_fact(fact, count)
        self.engine.add_changes(changes)

    def add_rule(self, rule):
        new_fact_counter = Counter()
        self.model.logger.info(rule)
        self.engine.add_rule(rule)


class IncrementalExecuter:
    def __init__(self, model):
        self.model = model
    ...
    def merge_delta_data(self):
        ...
        :return:
        """
        for index in self.model.type_index_map.values():
    ...
    def insert_delta_facts(self, facts_dict):
        ...
        for fact in facts_dict:
            ...
            self.model.type_index_map[fact.sort.name].add_delta_fact(fact, count)
    ...
    def print_bindings_list(self, bindings_counter):
        ...
        if len(bindings_counter) == 0:
            self.model.logger.debug('No bindings available for current rule.')
        else:
            self.model.logger.debug(str(bindings_counter))

    def execute_rule(self, rule):
        new_fact_counter = Counter()
        delta_rules = rule.derive_delta_rules()
        for delta_rule in delta_rules:
            self.model.logger.info(delta_rule)
            bindings_counter = delta_rule.find_match(self.model.type_index_map)
            ...
            for bindings in bindings_counter:
                ...
                for constraint in delta_rule.head:
                    ...
                    self.model.logger.debug('%s -> %s' % (fact, bindings_count))
        ...
        if rule.has_recursion:
            ...
            for fact in new_fact_counter:
                if fact not in self.model.type_index_map[fact.sort].data:
